 ...
 * @date   12 Sep 2017
 *
 * @brief  Defined a strongly defines proxy that iterates efficiently over a
 * field
 *
  ...
  namespace internal {
    ...
    class FieldMap<FieldCollection, T, NbComponents, ConstField>::iterator {
     ...
     public:
      ...
      //! stl conformance
      using value_type = const_corrector_t<FullyTypedFieldMap, ConstIter>;
      //! stl conformance
      using const_value_type = const_corrector_t<FullyTypedFieldMap, true>;
      //! stl conformance
      ...
      using TypedRef = std::conditional_t<ConstIter, const FullyTypedFieldMap,
                                          FullyTypedFieldMap> &;
      ...
      friend std::ostream &operator<<(std::ostream &os, const iterator &it) {
        ...
        os << "iterator on field '" << it.fieldmap.get_name() << "', entry "
           << it.index;
        return os;
      }
    ...
    class FieldMap<FieldCollection, T, NbComponents,
                   ConstField>::enumerator<SimpleIterator>::iterator {
     public:
      //! cell coordinates type
      using Ccoord = typename FieldCollection::Ccoord;
      //! stl conformance
      using value_type =
          std::tuple<Ccoord, typename SimpleIterator::value_type>;
      //! stl conformance
      using const_value_type =
          std::tuple<Ccoord, typename SimpleIterator::const_value_type>;
      //! stl conformance
      using difference_type = std::ptrdiff_t;
      //! stl conformance
      using iterator_category = std::random_access_iterator_tag;
      ...
      //! dereference
      value_type operator*() { return value_type{it.get_ccoord(), *this->it}; }
  ...
  namespace internal {
    ...
    FieldMap<FieldCollection, T, NbComponents, ConstField>::FieldMap(
        ...
        : collection(field.get_collection()),
          field(static_cast<TypedField_t &>(field)) {
      static_assert(
          std::is_same<FC, FieldCollection>::value,
    ...
    template <class FieldCollection, typename T, Dim_t NbComponents,
              bool ConstField>
    void FieldMap<FieldCollection, T, NbComponents,
                  ConstField>::check_compatibility() {
      if (typeid(T).hash_code() !=
          this->field.get_stored_typeid().hash_code()) {
        std::string err{"Cannot create a Map of type '" + this->info_string() +
                        "' for field '" + this->field.get_name() +
                        "' of type '" + this->field.get_stored_typeid().name() +
                        "'"};
        throw FieldInterpretationError(err);
      }
      // check size compatibility
      if ((NbComponents != Dim_t(this->field.get_nb_components())) and
          (NbComponents != Eigen::Dynamic)) {
        throw FieldInterpretationError(
            "Cannot create a Map of type '" + this->info_string() +
    ...
    template <class FieldCollection, typename T, Dim_t NbComponents,
              bool ConstField>
    size_t
    FieldMap<FieldCollection, T, NbComponents, ConstField>::size() const {
    ...
    struct FieldMap<FieldCollection, T, NbComponents,
                    ConstField>::is_compatible {
      //! creates a more readable compile error
      constexpr static bool explain() {
        static_assert(
            std::is_same<typename myField::collection_t,
                         FieldCollection>::value,
            "The field does not have the expected FieldCollection type");
        static_assert(std::is_same<typename myField::Scalar, T>::value,
                      "The // field does not have the expected Scalar type");
        static_assert(
            (TypedField_t::nb_components == NbComponents),
            "The field does not have the expected number of components");
        // The static asserts wouldn't pass in the incompatible case, so this is
        // it
        return true;
      }
      //! evaluated compatibility
      constexpr static bool value{
          std::is_base_of<TypedField_t, myField>::value};
    };
    ...
    template <class FieldCollection, typename T, Dim_t NbComponents,
              bool ConstField>
    const std::string &
    FieldMap<FieldCollection, T, NbComponents, ConstField>::get_name() const {
    ...
    FieldMap<FieldCollection, T, NbComponents, ConstField>::get_collection()
        const {
    ...
    FieldMap<FieldCollection, T, NbComponents, ConstField>::iterator<
        FullyTypedFieldMap, ConstIter>::iterator(TypedRef fieldmap, bool begin)
    ...
    template <class FullyTypedFieldMap, bool ConstIter>
    FieldMap<FieldCollection, T, NbComponents, ConstField>::iterator<
        FullyTypedFieldMap, ConstIter>::iterator(TypedRef fieldmap,
                                                 size_t index)
        : collection(fieldmap.collection), fieldmap(fieldmap), index(index) {}
    ...
    typename FieldMap<FieldCollection, T, NbComponents, ConstField>::
        template iterator<FullyTypedFieldMap, ConstIter> &
    ...
    FieldMap<FieldCollection, T, NbComponents, ConstField>::iterator<
        FullyTypedFieldMap, ConstIter>::get_ccoord() const {
      return this->collection.get_ccoord(this->index);
    }

    ////----------------------------------------------------------------------------//
    // template<class FieldCollection, typename T, Dim_t NbComponents, class
    // FullyTypedFieldMap> std::ostream & operator << (std::ostream &os,
    ...
    FieldMap<FieldCollection, T, NbComponents, ConstField>::get_ptr_to_entry(
        size_t index) {
