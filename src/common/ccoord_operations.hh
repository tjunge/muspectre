...
template <typename T> constexpr T ret(T val, size_t /*dummy*/) { return val; }
...
constexpr Ccoord_t<dim> get_default_strides(const Ccoord_t<dim> &sizes) {
  return internal::compute_strides<dim>(sizes, std::make_index_sequence<dim>{});
}
...
constexpr Ccoord_t<dim> get_ccoord(const Ccoord_t<dim> &resolutions,
                                   const Ccoord_t<dim> &locations, Dim_t index,
                                   std::index_sequence<I...>) {
...
template <size_t dim> constexpr size_t get_size(const Ccoord_t<dim> &sizes) {
  Dim_t retval{1};
  for (size_t i = 0; i < dim; ++i) {
    retval *= sizes[i];
  }
 ...
 * transposed
 */
template <size_t dim, int... dmap> class Pixels {
 public:
  //! constructor
...
typename Pixels<dim, dmap...>::iterator::value_type
  ...
  return get_ccoord(
      pixels.resolutions, pixels.locations, this->index,
      std::conditional_t<sizeof...(dmap) == 0, std::make_index_sequence<dim>,
                         std::index_sequence<dmap...>>{});
}

/* ---------------------------------------------------------------------- */
template <size_t dim, int... dmap>
bool Pixels<dim, dmap...>::iterator::operator!=(const iterator &other) const {
