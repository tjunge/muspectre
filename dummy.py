...
def reference_curve (tauy0_input, h_input, max_shear, E, nu,
    ...
    Ny     = 5          # number of voxels in y-direction
    Nz     = 3          # number of voxels in z-direction
    ...
    K_input = E/(3*(1-2*nu))
    mu_input = E/(2*(1+nu))
    print("K_input = {}, mu_input = {}".format(K_input, mu_input))
    # material parameters
    K      = param(0.833,K_input)  # bulk      modulus
    mu     = param(0.386,mu_input)  # shear     modulus
    H      = param(0.004,h_input)  # hardening modulus
    tauy0  = param(0.003,tauy0_input)  # initial yield stress
    ...
    ninc   = int(max_shear/shear_incr)
    lam    = 0.0
    # for directional stiffness in shear direction
    del_tau = np.array(I,copy=True)*0
    del_tau[0, 1] = 1.
    # for directional stiffness in σₓₓ direction
    del_sigxx = np.array(I,copy=True)*0
    del_sigxx[0, 0] = 1.
    ...
    gammas=np.zeros(ninc, dtype=float)
    tau = np.zeros_like(gammas)
    ...
    for inc in range(ninc):
        ...
        dsigma_xx[inc] = K_dF(del_sigxx)[0,0,0,0,0]
