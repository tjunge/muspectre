...
def deserialise_t4(t4, dim=2):
    turnaroud = np.arange(dim**2).reshape(dim,dim).T.reshape(-1)
    retval = np.zeros([dim*dim, dim*dim])
    for i,j in itertools.product(range(dim**2), repeat=2):
        retval[i,j] = t4[:dim, :dim, :dim, :dim, 0,0].reshape(dim**2, dim**2)[turnaroud[i], turnaroud[j]]
        pass
    return retval

def t2_to_goose(t2_msp, dim):
    t2_goose = np.zeros((dim,dim,Nx, Ny))
    for i in range(Nx):
        for j in range(Ny):
            t2_goose[:,:,i,j] = t2_msp[:, i + Nx*j].reshape(dim, dim).T
            pass
        pass
    return t2_goose

def t2_vec_to_goose(t2_msp_vec, dim):
    return t2_to_goose(t2_msp_vec.reshape(dim*dim, Nx*Ny)).reshape(-1)

def t2_from_goose(t2_goose, dim):
    nb_pix = Nx*Ny
    t2_msp = np.zeros((dim**2, nb_pix), order='F')
    for i in range(Nx):
        for j in range(Ny):
            view = t2_msp[:, i + Nx*j].reshape(dim, dim).T
            view = t2goose[:,:,i,j].T
            pass
        pass
    return t2_msp
...
#---------------------------- µSpectre init -----------------------------------
resolution = list(phase.shape); resolution.append(1)
dim = len(resolution)
...
mspbarF    = mspF.copy()
mspbarF_t  = mspF.copy()
mspP, mspK = rve.evaluate_stress_tangent(mspF)
mspG_K_dF = lambda x: rve.directional_stiffness(x.reshape(F.shape)).reshape(-1)
mspG = lambda x: rve.project(x).reshape(-1)
...
for inc,lam in zip(range(1,ninc+1),stretch[:24]):
    ...
    while True:
        ...
        if i: raise IOError('CG-solver failed, i = {}'.format(i))
        ...
        print('norm: {0:10.9e}, nb_newon = {1}'.format(np.linalg.norm(dFm)/Fn, iiter))
