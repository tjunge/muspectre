...
namespace muSpectre {
  ...
  using fixlist = boost::mpl::list<
      CellBaseFixture<twoD, twoD, Formulation::finite_strain>,
      CellBaseFixture<threeD, threeD, Formulation::finite_strain>,
      CellBaseFixture<twoD, twoD, Formulation::small_strain>,
      CellBaseFixture<threeD, threeD, Formulation::small_strain>>;
  ...
  BOOST_FIXTURE_TEST_CASE_TEMPLATE(test_globalised_fields, Fix, fixlist, Fix) {
    ...
    auto &compatible_soft{
        make_field<TypedField<LColl_t, Real>>(compatible_name, col_soft, Dim)};
    auto &compatible_hard{
        make_field<TypedField<LColl_t, Real>>(compatible_name, col_hard, Dim)};
    ...
    for (auto &&tup : glo_map.enumerate()) {
      ...
      Real err{
          (val - Map_t(pixel.data()).template cast<Real>()).matrix().norm()};
      BOOST_CHECK_LT(err, tol);
    }

    // incompatible fields:
    const std::string incompatible_name{"incompatible"};
    make_field<TypedField<LColl_t, Real>>(incompatible_name, col_soft, Dim);

    make_field<TypedField<LColl_t, Real>>(incompatible_name, col_hard, Dim + 1);
    BOOST_CHECK_THROW(
        this->get_globalised_internal_real_field(incompatible_name),
        std::runtime_error);
    ...
    make_field<TypedField<LColl_t, Real>>(wrong_scalar_name, col_soft, Dim);

    make_field<TypedField<LColl_t, Dim_t>>(wrong_scalar_name, col_hard, Dim);
    BOOST_CHECK_THROW(
        this->get_globalised_internal_real_field(wrong_scalar_name),
        std::runtime_error);
  }
