...
namespace muSpectre {
  ...
  BOOST_FIXTURE_TEST_CASE_TEMPLATE(test_globalised_fields, Fix, fixlist, Fix) {
    ...
    make_field<TypedField<LColl_t, Real>>(incompatible_name,
                                          col_hard,
                                          Dim+1);
    BOOST_CHECK_THROW(this->get_globalised_internal_field(incompatible_name),
                      std::runtime_error);

    // wrong name/ inexistant field
    const std::string wrong_name{"wrong_name"};
    BOOST_CHECK_THROW(this->get_globalised_internal_field(wrong_name),
                      std::runtime_error);
    ...
    make_field<TypedField<LColl_t, Dim_t>>(wrong_scalar_name,
                                           col_hard,
                                           Dim);
    BOOST_CHECK_THROW(this->get_globalised_internal_field(wrong_scalar_name),
                      std::runtime_error);
  }
